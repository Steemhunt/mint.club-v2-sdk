...
```ts
await mintclub.network('ethereum').nft('MNFT') // [!code focus]
```
...
:::warning[IPFS]
Please refer to [uploadMetadata](/docs/sdk/ipfs/uploadMetadata) for more information on how to upload metadata to IPFS.

If you have a own server that manages the metadata and want to use a public URL for the metadata, you can pass a `https` url.
:::

:::code-group

```tsx [curvedata-example.tsx]
...
await mintclub
  ...
  .create({
    ...
    curveData,
    metadataUrl: 'ipfs://...' | 'https://...', // [!code focus]
  })
```
...
:::warning[APPROXIMATE CURVE GENERATION]
When using `curveData`, the SDK will do its best to generate a bonding curve that approximates your desired parameters. However, due to the nature of curve generation algorithms and on-chain constraints, there may be minor discrepancies between the desired prices and the actual generated prices.

If you need precise control over the exact step ranges and prices, please use [`stepData`](#stepdata-conditionally-required) instead.
:::
