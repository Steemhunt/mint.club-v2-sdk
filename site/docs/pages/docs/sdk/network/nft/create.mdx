...
- Type: `Buffer` | `'ipfs://${string}'` | `'https://${string}'`

The image to use for the nft. You can pass a `Buffer`, an `IPFS Hash`, or a `URL`.
...
await mintclub
  ...
  .create({
    ...
    // or a Buffer instance when using in node.js // [!code focus]
    curveData,
  })
```
...
The animation to use for the nft. You can pass a `Buffer`, an `IPFS Hash`, or a `URL`.
...
This callback function will only be invoked when you pass a `Buffer` for the `image` or `video` parameter.
...
This callback function will only be invoked when you pass a `Buffer` for the `image` or `video` parameter along with the filebaseApiKey.
