---
description: Sells NFTs from the bonding curve contract.
---

# sell 

Sells NFTs from the bonding curve contract.
...
await mintclub.network('ethereum').nft('SYMBOL').sell({
	amount: wei(1, 18),
})
```
...
The amount of NFTs to sell from the bonding curve contract.
...
:::warning[Tip]
Unlike ERC-20 tokens, NFT approvals are either `true` or `false`. 

So `allowanceAmount` is not needed.
:::
