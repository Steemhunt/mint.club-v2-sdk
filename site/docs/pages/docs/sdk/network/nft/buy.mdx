---
description: Buys NFTs from the bonding curve contract.
---

# buy 

Buys NFTs from the bonding curve contract.

:::tip[tip]
Before user makes a `buy` call, the bonding curve contract must be `approved` to use the asset's reserve token.
...
await mintclub.network('ethereum').nft('SYMBOL').buy({
	amount: wei(1, 18),
})
```
...
The amount of NFTs to buy from the bonding curve contract.
...
The address to send the bought NFTs to. If not provided, the NFTs will be sent to the user's address.
...
The amount of NFTs to approve the bonding curve contract to use the asset's reserve token. If not provided, the SDK will automatically use `MaxUint256` as the allowance amount.
