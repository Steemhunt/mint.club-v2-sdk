...
- Non–Mint Club tokens: priced via 1inch on the current chain. For tokens with very small values (< $0.000001), the SDK automatically applies precision-preserving fallbacks:
  - reverse quote: STABLE → TOKEN (invert to USD per token)
  - or TOKEN → ETH → USD path
...
- Returns `usdRate: null` when no supported quote path is available.
