# 🥩 Staking Operations — Complete LLM Guide
...
> Direct LLM guideline (plain text): [stake-llm.txt](https://sdk.mint.club/stake-llm.txt)
...
## 📋 Complete Staking Operations Guide
...
await sdk.network('base').stake.emergencyUnstake({
  ...
  onSuccess: (receipt) => {
    console.log('🚨 Emergency unstaked - rewards forfeited')
...
console.log('📊 Total claimed so far:', totalClaimed)
console.log('💸 Total fees paid:', totalFees)

// Check if worth claiming
if (rewardAmount > claimFee) {
  console.log('✅ Worth claiming (rewards > fee)')
} else {
  console.log('⏳ Not worth claiming yet (fee > rewards)')
}
```
...
console.log('⏰ Last stake time:', new Date(lastStakeTime * 1000))
```
...
await sdk.network('base').stake.updateCreationFee({
  ...
  onSuccess: () => console.log('✅ Creation fee updated'),
})

// Update claim fee
await sdk.network('base').stake.updateClaimFee({
  fee: 300, // 3% claim fee
  onSuccess: () => console.log('✅ Claim fee updated'),
})
```
...
await sdk.network('base').stake.updateProtocolBeneficiary({
  ...
  onSuccess: () => console.log('✅ Beneficiary updated'),
})
```
...
await sdk.network('base').stake.renounceOwnership({
  onSuccess: () => console.log('✅ Ownership renounced'),
})
```
...
await sdk.network('base').stake.cancelPool({
  poolId: 0,
  onSuccess: (receipt) => {
    console.log('✅ Pool cancelled, remaining rewards returned to creator')
  },
  onError: (error) => {
    console.error('❌ Pool cancellation failed:', error)
  },
})
```
...
class AdvancedStakingManager {
  ...
  async createAndManagePool(poolConfig: {
    ...
    try {
      // 1. Create the pool
      console.log('🏗️ Creating staking pool...')
      ...
      console.error('❌ Pool creation/management failed:', error)
      throw error
    }
  }
  ...
  async optimizeStakingStrategy(poolId: number) {
    try {
      ...
      if (timeRemaining < 86400) {
        // Less than 1 day
        console.log('⏰ Pool ending soon - consider claiming rewards')
      }

      if (pool.rewardRemaining < pool.rewardAmount * 0.1) {
        // Less than 10% remaining
        console.log('⚠️ Low rewards remaining - consider unstaking')
      }
...
class StakingAnalytics {
    ...
    try {
      ...
      for (const pool of allPools) {
        try {
          ...
          console.log(`⚠️ Could not analyze pool ${pool.id}:`, error.message)
        }
      }
      ...
      console.log('📊 Staking Report for:', userAddress)
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  ...
  async findBestStakingOpportunities() {
    try {
      ...
      console.log('🏆 Best Staking Opportunities:')
...
if (currentTime < pool.rewardStartsAt) {
  ...
  console.log(`⏰ Rewards start in ${Math.ceil(timeUntilStart / 3600)} hours`)
  return
}
...
if (currentTime > rewardEndTime) {
  console.log('⏰ Reward period has ended')
  return
}
```
...
if (rewardAmount <= claimFee) {
  console.log('⏳ Rewards too small to claim profitably')
...
if (currentTime < lastStakeTime + lockPeriod) {
  ...
  console.log(`⏰ Cannot unstake yet. ${Math.ceil(timeUntilUnlock / 86400)} days remaining`)
}
```
...
if (pool.rewardRemaining === 0n && pool.totalStaked === 0n) {
  console.log('⚠️ Pool appears to be inactive or cancelled')
}
```
...
async function confirmUnstaking(poolId: number, amount: string) {
  ...
  if (rewardAmount > claimFee) {
    console.log('⚠️ Warning: You have unclaimed rewards!')
