...
await sdk
  ...
  .create({
    ...
    buyRoyalty: 300, // 3%
    sellRoyalty: 300, // 3%
    creatorAllocation: wei(10000, 18), // 10k tokens for creator
    onSuccess: (receipt) => {
...
interface TokenCreateParams {
  ...
  curveData: {
    ...
    creatorAllocation?: bigint // Tokens allocated to creator
  }
  buyRoyalty?: number // Buy royalty in basis points (0-10000)
  sellRoyalty?: number // Sell royalty in basis points (0-10000)
...
await sdk
  ...
  .buy({
    ...
    slippage: 100, // Slippage tolerance (1%)
...
await sdk
  ...
  .buyWithZap({
    amount: wei(100, 18),
    slippage: 200, // Higher slippage for zap (2%)
    onSuccess: (receipt) => {
...
// 3. Include creator allocation for liquidity
creatorAllocation: wei(50000, 18) // 5% for creator/liquidity

// 4. Set appropriate royalties
buyRoyalty: 300,   // 3% buy royalty
sellRoyalty: 300   // 3% sell royalty
```
...
const slippage = marketVolatility > 0.1 ? 500 : 100 // Higher slippage for volatile markets
