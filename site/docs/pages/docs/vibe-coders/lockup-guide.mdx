# 🔒 Token Lockup Operations — Complete LLM Guide
...
## 📋 Complete Lockup Operations Guide
...
await sdk.network('base').lockup.createLockUp({
  ...
  onSuccess: (receipt) => {
    console.log('✅ Tokens locked successfully:', receipt.transactionHash)
  },
  onError: (error) => {
    console.error('❌ Lockup creation failed:', error)
  },
})
```
...
await sdk.network('base').lockup.unlock({
  ...
  onSuccess: (receipt) => {
    console.log('✅ Tokens unlocked and transferred')
...
async function batchUnlockLockups(sdk: any, lockupIds: number[]) {
  for (const lockupId of lockupIds) {
    try {
      ...
      if (lockup.unlocked) {
        console.log(`⏭️ Lockup ${lockupId} already unlocked`)
        continue
      }

      const currentTime = Math.floor(Date.now() / 1000)
      if (currentTime < lockup.unlockTime) {
        console.log(`⏰ Lockup ${lockupId} not ready yet`)
      ...
      await sdk.network('base').lockup.unlock({
        lockUpId,
        onSuccess: (receipt) => {
          console.log(`✅ Unlocked lockup ${lockupId}`)
        },
      })
      ...
      console.error(`❌ Failed to unlock lockup ${lockupId}:`, error)
    }
  }
}
```
    ...
    for (const lockup of eligibleLockups) {
      try {
        await this.sdk.network('base').lockup.unlock({
          lockUpId: lockup.id,
          onSuccess: (receipt) => {
            console.log(`✅ Unlocked ${lockup.title}`)
          },
        })
...
async function analyzeLockup(sdk: any, lockupId: number) {
  ...
  try {
    ...
    console.error('❌ Lockup analysis failed:', error)
    throw error
  }
}
```
...
if (details.unlocked) {
  console.log('⏭️ Lockup already unlocked')
...
try {
  const details = await sdk.network('base').lockup.getLockUpById(lockupId)
  console.log('✅ Lockup exists:', details.title)
} catch (error) {
  console.error('❌ Lockup not found:', lockupId)
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      await sdk.network('base').lockup.unlock({
        lockUpId,
        onSuccess: (receipt) => {
          console.log(`✅ Unlocked on attempt ${attempt}`)
        },
      })
...
async function checkAndNotifyUnlocks(sdk: any, userAddress: string) {
  ...
  if (readyToUnlock.length > 0) {
    console.log('🎉 Tokens ready to unlock!')
...
async function createMultisigLockup(sdk: any, multisigAddress: string, amount: string, unlockTime: number) {
  ...
  console.log('⚠️ Large lockup requires multisig approval')
...
**AI Assistants:** This guide covers every aspect of token lockup operations from creation to unlocking. Remember to always validate unlock times, implement proper error handling, and consider gas costs for bulk operations. For large-scale vesting programs, consider breaking operations into smaller batches! 🔒
