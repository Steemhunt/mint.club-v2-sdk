...
const CHAINS$1 = [
  {
    ...
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.mainnet.id])
  },
  {
    id: chains.base.id,
    name: "Base",
    ...
    color: "#0052FF",
    openseaSlug: "base",
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.base.id])
  },
  {
    ...
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.optimism.id])
  },
  {
  ...
  },
  {
    ...
    color: "#8247E5",
    openseaSlug: "matic",
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.polygon.id])
  },
  {
    id: chains.bsc.id,
    name: "BNBChain",
    ...
    color: "#F0B90B",
    openseaSlug: "bsc",
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.bsc.id])
  },
  {
    ...
    color: "#627EEA",
    openseaSlug: "sepolia",
    enabled: viem.isAddress(CONTRACT_ADDRESSES.BOND[chains.sepolia.id]),
    isTestnet: true
  }
];
...
class GenericContractLogic {
  ...
  constructor(params) {
    ...
    const chain = Object.values(chains__namespace).find((chain2) => chain2.id === chainId);
    if (!chain)
    ...
    this.publicClient = viem.createPublicClient({
      chain,
      transport: viem.fallback(chainRPCFallbacks(chain.id), { rank: true }),
      ...options
    });
  }
  withConfig(options) {
    const chain = Object.values(chains__namespace).find((chain2) => chain2.id === this.chainId);
    if (!chain)
      throw new Error("Chain  not found");
    this.publicClient = viem.createPublicClient({
      chain,
      transport: viem.fallback(chainRPCFallbacks(this.chain.id), { rank: true }),
      ...options
    });
  }
...
exports.merkleContract = merkleContract;
